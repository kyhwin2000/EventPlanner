...
var drawCalendar = function(y,m){ 
	...
	for(var i=0;i<tile.length;i++){
		...
		tile[i].addEventListener('click',function(e){
			...
			this.color = '#008E00';
			surveyDate.push(parseInt(this.getTitle()));
		});
		//타일을 뷰에 더하기 
		curWin.add(tile[i]);	
	}
...
{
	// 약속명과 선택된 날짜들을 로컬 프로퍼티에 저장하기 
	Ti.App.Properties.setString('eventName',tf1.value); 
	Ti.App.Properties.setList('surveyDate',surveyDate);
	// 약속명과 선택된 날짜들을 서버로 보내기
